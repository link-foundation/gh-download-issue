...
async function fetchIssue(owner, repo, issueNumber, token) {
  try {
    ...
    log('green', `✅ Successfully fetched issue with ${comments.length} comments`)
...
function issueToMarkdown(issueData, commentsData) {
  ...
  if (issue.assignees && issue.assignees.length > 0) {
    const assignees = issue.assignees.map(a => `[@${a.login}](${a.html_url})`).join(', ')
...
const argv = yargs(hideBin(process.argv))
  ...
  .command('$0 <issue>', 'Download a GitHub issue and convert it to markdown', (yargs) => {
  ...
  .option('output', {
    alias: 'o',
    type: 'string',
    describe: 'Output file path (default: issue-<number>.md in current directory)'
  })
  .help('h')
  .alias('h', 'help')
...
async function main() {
  let { issue: issueInput, token, output } = argv

  // Parse the issue URL
  const parsed = parseIssueUrl(issueInput)
  if (!parsed) {
    log('red', '❌ Invalid issue URL or format')
    log('yellow', '   Expected: https://github.com/owner/repo/issues/123 or owner/repo#123')
    process.exit(1)
  }
  ...
  const outputPath = output || path.join(process.cwd(), `issue-${issueNumber}.md`)
